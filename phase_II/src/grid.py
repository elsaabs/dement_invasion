...
from microbe import microbe_osmo_psi                # that's the 2 functions after the Microbe class
from microbe import microbe_mortality_prob as MMP
from enzyme  import Arrhenius
from utility import expand
from utility import temp_seed
...
class Grid():
    ...
    def __init__(self,runtime,data_init): 
        ...
        # Parameters specific to the invasion simulation that are in runtime
        ...
        self.x                 = int(runtime.loc['x',1])
        self.y                 = int(runtime.loc['y',1])
        
        # Degradation
        self.n_enzymes         = data_init['n_enzymes']
        ...
        self.Microbes          = data_init['Microbes'].copy(deep=True)      # microbial community at the beginning of invasion
        self.TaxUpt            = data_init['TaxUpt']                        # Transporter that each taxon has, expanded on grid
        self.n_taxa            = data_init['n_taxa']                        # Number of resident + invader taxa at the beginning of invasion
        ...
        self.direct            = runtime.loc['direct',1]                    # dispersal direction: 0.95
        
        # Repopulation
        self.Monomers_init     = data_init['Monomers'].copy(deep=True)      # Monomers
    ...
    def mortality(self,day):       
        ...
        Death_gridcell = Death.groupby(Death.index//self.n_taxa,axis=0).sum()   # Nadya's suggestion
    ...
    def reproduction(self,day):           
        ...
        bac_index = (self.fb==0) & (self.Microbes['C']>self.max_size_b) # We target all individuals that are not fungi and which biomass is > division threshold.
        ...
        Reprod_index_series = Reprod_index_series.rename(columns={0: "index_series"})  #r ename the column
    ...
    def repopulation(self,day):
        ...
        if mic_reinit == True:
            ...
            self.Microbes_init = self.Microbes.copy(deep=True) #NOTE copy()
            ...
            Taxon_index_764 = self.Microbes['C'] > 0
            Taxon_count_repop_764 = Taxon_index_764.groupby(level=0,sort=False).sum().astype('uint32')
            #n_taxa_presents = len(self.Microbes[Microbe.iloc[:,21] != 0])
            """
