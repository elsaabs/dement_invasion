...
class Monomer():
    ...
    def monomer_uptake_reqenzyme(self):
        ...
        # PIECE#4 - Mise en commentaire pour augmenter la selection
        """
        ...
        # Elsa's code proposition #1: Upt1 for NO4 and PO4, Upt2-14 for the other monomers (so that no taxa gain monomers thanks to having Upt for NO4 and PO4, but they can still gain monomers from other Upts)
        """
        # je veux que les transporteurs de Mon1 et Mon2 ne transportent que Mon1 et Mon2
        # c'est le meme transporteur qui prend NO4 et PO4 et il ne prend aucun autre MonX
        # (1) tableau a (n_uptake - 1) colonnes
        probability_list = [0] * (self.n_uptake - 1)
        probability_list[0:self.Uptake_per_monomer] = [1]*self.Uptake_per_monomer
        Uptake_ReqEnz_list = [np.random.choice(probability_list,(self.n_uptake - 1),replace=False) for i in range((self.n_monomers - 2))]
        # (2) tu le transformes en tableau
        Uptake_ReqEnz_list_df = pd.DataFrame(data=np.array(Uptake_ReqEnz_list).reshape((self.n_monomers - 2),(self.n_uptake - 1)),dtype='int8')
        # (3) create a column of (self.n_monomers - 2)x0
        first_column = [0] * (self.n_monomers - 2)
        # (4) insert the column in 1st position
        Uptake_ReqEnz_list_df.insert(loc=0, column='A', value=first_column)
        # (5) change columns name for (0,self.n_uptake)
        Uptake_ReqEnz_list_df.columns = range(Uptake_ReqEnz_list_df.shape[1])
        # (6) rajoutes 2 rows pour Mon1 et Mon 2 = (1,0,...0)
        first_row = [0] * (self.n_uptake)
        first_row[0] = 1
        data = []
        data.insert(0, first_row)
        data.insert(0, first_row)
        Uptake_ReqEnz_list_df_final = pd.concat([pd.DataFrame(data), Uptake_ReqEnz_list_df], ignore_index=True)
        # (7) rechange index et nom des colonnes
        index   = ["Mon" + str(i) for i in range(1,self.n_monomers+1)]
        columns = ['Upt' + str(i) for i in range(1,self.n_uptake+1)]
        Uptake_ReqEnz_df = pd.DataFrame(data=Uptake_ReqEnz_list_df_final.values,index=index,columns=columns,dtype='int8')
        """
        
        # Elsa's code proposition #2: 1 Upt per Mon (diagonal matrix)
        # je veux que Uptake_ReqEnz soit une matrice diagonale
        flatted_input = [1] * (self.n_uptake)
        index   = ["Mon" + str(i) for i in range(1,self.n_monomers+1)]
        columns = ['Upt' + str(i) for i in range(1,self.n_uptake+1)]
        Uptake_ReqEnz_df = pd.DataFrame(data=np.diagflat(flatted_input),index=index,columns=columns,dtype='int8')
