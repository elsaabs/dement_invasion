...
def initialize_data(substrates_init, runtime_parameters):
    """
    Parameters:
    ...
    seed_number       = 210121234 + int(seed_index) 
    ...
    #...Below is "MicrobeS" (with an S) because I defined Microbe class as such just above. And I imported Microbe class from microbe.py line 12.
    ...
    microbial_uptake_monomer = Microbes.microbe_uptake_monomer(microbial_uptake_gene[1],monomers_uptake_reqenzyme)
    #...Microbial uptake cost
    microbial_uptake_cost = Microbes.microbe_uptake_cost(microbial_uptake_gene[1])
    ...
    n_taxa = int(runtime_parameters.loc['n_taxa',1])
    n_enzymes = int(runtime_parameters.loc['n_enzymes',1])
    n_substrates = int(runtime_parameters.loc['n_substrates',1])
    n_uptake = int(runtime_parameters.loc['n_uptake',1])
    n_osmolytes = int(runtime_parameters.loc['n_osmolytes',1])
    fb = float(runtime_parameters.loc['fb',1])
    taxa_per_box = float(runtime_parameters.loc['taxa_per_box',1])
    gridsize = int(runtime_parameters.loc['gridsize',1])
    x = int(runtime_parameters.loc['x',1])
    y = int(runtime_parameters.loc['y',1])
    
    #Checking that I could pull out the initial microbial community like below (it works ;))
    #print('microbial_community[1] = ', microbial_community[1])
    
    #Checking if I need to call return[0] or just return when there is only one return (it works ;))
    #print('microbial_min_ratios = ', microbial_min_ratios)
    
    Data_Dictionary = {"seed_index": seed_index,
                       "seed_number": seed_number,
                       "n_taxa": n_taxa,
                       "n_enzymes": n_enzymes,
                       "n_substrates": n_substrates,
                       "n_uptake": n_uptake,
                       "n_osmolytes": n_osmolytes,
                       "fb_proba": fb,
                       "taxa_per_box": taxa_per_box,
                       "gridsize": gridsize,
                       "grid_x": x,
                       "grid_y": y,
                       ...
                       "Uptake_ReqEnz_nonspatial":monomers_uptake_reqenzyme,
                       ...
                       "Uptake_Km0_nonspatial":   enzymes_uptake_Km,           # transporter half-saturation constant, not expanded on grid
                       ...
                       "Uptake_Vmax0_nonspatial": enzymes_uptake_Vmax,         # transporter reaction rate, not expanded on grid
                       ...
                       "TaxMon_enzymes": microbial_uptake_gene[0],          # monomers each taxon has access to through its enzymes
                       "TaxMon_intermed": microbial_uptake_gene[1],         # monomers each taxon has access to through its enzymes + monomers randomly added
                       "TaxUpt_nonspatial": microbial_uptake_monomer[0],                   # transporter that each taxon has
                       "TaxMon_final": microbial_uptake_monomer[1],                   # all monomers that each taxon can take up thanks to its enzymes + transporters + randomly added
                       "TaxUpt": expand(microbial_uptake_monomer[0],gridsize),     # transporter that each taxon has, expanded on grid
                       "UptakeGenes": expand(microbial_uptake_gene[1],gridsize),  # transporter gene distribution across taxa
    ...
    #print('Data_Dictionary["TaxMon_enzymes"] :', Data_Dictionary["TaxMon_enzymes"])
    #print('Data_Dictionary["Uptake_ReqEnz_nonspatial"] :', Data_Dictionary["Uptake_ReqEnz_nonspatial"])
    #print('Data_Dictionary["TaxMon_final"] :', Data_Dictionary["TaxMon_final"])
