...
class Microbe():
    ...
    def __init__(self,runtime,parameters):
        ...
        self.Enz_per_taxon_max = int(sys.argv[6])
    ...
    def microbe_enzyme_gene(self):
        ...
        #print('EnzGenes',EnzGenes[['Enz2','Enz48','Enz49']])
    ...
    def microbe_uptake_gene(self,ReqEnz,EnzGenes,MonomersProduced):
        ...
        TaxMon_enzymes = EnzGenes @ Sub_enz.T @ MonomersProduced
        TaxMon_enzymes.iloc[:,0:2] = np.int8(1)    # all taxa can take up NO4 and PO4
        TaxMon_enzymes[lambda df: df>0] = np.int8(1)  # == UptakeGenes[UptakeGenes > 0] = 1     # make sure there is no value > 1
        UptakeGenes = TaxMon_enzymes.copy(deep=True)

        # PIECE#2 - Mise en commentaire pour augmenter la selection
        """
        ...
        # PIECE#3 - Mise en commentaire pour augmenter la selection
        """
        ...
        return TaxMon_enzymes, UptakeGenes
        
        
        
        
    def microbe_uptake_monomer(self,TaxMon_intermed,Uptake_ReqEnz_nonspatial):
        """
        Derive which transporters each taxon has, and which monomers each taxon has access to through enzymes + randomly added + transporters.
        
        Parameters:
            TaxMon_intermed:
            Uptake_ReqEnz_nonspatial:
        Return:
            TaxUpt:
            TaxMon:
        """
    
        # TaxUpt tells me which transporters each taxon has
        TaxUpt_array = TaxMon_intermed.values @ Uptake_ReqEnz_nonspatial.values
        TaxUpt       = pd.DataFrame(data=TaxUpt_array, index=TaxMon_intermed.index, columns=Uptake_ReqEnz_nonspatial.T.index)
        TaxUpt[TaxUpt > 0] = 1
        
        # TaxMon tells me the final relative amount of Monomers each Taxon can get thanks to its Transporters
        TaxMon = TaxUpt @ Uptake_ReqEnz_nonspatial.T
    ...
    def microbe_uptake_cost(self,UptakeGenes):                         
        ...
        index = ["Tax" + str(i) for i in range(1,self.n_taxa+1)]
        np.random.seed(3); UptakeProd_array  = LHS(self.n_taxa, self.Uptake_C_cost_min, self.Uptake_C_cost_max, 'uniform')
